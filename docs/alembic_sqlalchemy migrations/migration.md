...
3. Create a migration environment.
    ```
    alembic init alembic_sqlalchemy
    ```
    It will generate a migration directory `alembic_sqlalchemy` and `alembic_sqlalchemy.ini` file in project root directory.
4. Edit `alembic_sqlalchemy.ini` file for databse URL. Replace the database URL at `sqlalchemy.url` field in `alembic_sqlalchemy.ini` file with your database URL. In this case:
    `sqlalchemy.url = mysql+pymysql://dbuser:dbpassword@localhost:3306/order_management`

5. Go to newly created `alembic_sqlalchemy` directory. Modify `env.py` file as:
...
6. Create a new revision using command:
    ...
    OR
    alembic -c alembic_sqlalchemy.ini revision --autogenerate -m "Initial migration"
    ```
7. If you get error like `ERROR [alembic.util.messaging] Target database is not up to date.`, run following command and then again try to create revision:
    ...
    OR
    alembic -c alembic_sqlalchemy.ini upgrade head
    ```

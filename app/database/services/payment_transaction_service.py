...
def search_payment_transactions(session: Session, filter) -> PaymentTransactionSearchResults:

    query = session.query(PaymentTransaction)
    if filter.DisplayCode :
        query = query.filter(PaymentTransaction.DisplayCode == filter.DisplayCode)
    ...
    if filter.CustomerId:
        query = query.filter(PaymentTransaction.CustomerId == filter.CustomerId)
    if filter.OrderId:
        query = query.filter(PaymentTransaction.OrderId == filter.OrderId)
    if filter.PaymentMode:
        query = query.filter(PaymentTransaction.PaymentMode == filter.PaymentMode)
    if filter.PaymentAmount:
        query = query.filter(PaymentTransaction.PaymentAmount == filter.PaymentAmount)
    payment_transactions = query.all()

    results = PaymentTransactionSearchResults(
        TotalCount=len(payment_transactions),
        ItemsPerPage=filter.ItemsPerPage,
        PageNumber=filter.PageNumber,
        OrderBy=filter.OrderBy,
        OrderByDescending=filter.OrderByDescending,
        Items=payment_transactions
    )

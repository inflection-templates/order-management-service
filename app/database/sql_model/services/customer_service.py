...
from app.database.sql_model.models.customer import Customer
from app.database.sql_model.models.customer_address import CustomerAddress
...
from sqlmodel import Session, func, select, desc, asc
...
def create_customer(session: Session, model: CustomerCreateModel) -> CustomerResponseModel:
    ...
    if model.Email != None and model.Email != "":
        existing_customer = session.exec(select(Customer).filter(func.lower(Customer.Email) == func.lower(model.Email))).first()
        if existing_customer:
            raise Conflict(f"Customer with email {model.Email} already exists!")

    if model.Phone != None and model.Phone != "":
        existing_customer = session.exec(select(Customer).filter(Customer.Phone == model.Phone)).first()
        if existing_customer:
            raise Conflict(f"Customer with phone {model.Phone} already exists!")

    if model.TaxNumber != None and model.TaxNumber != "":
        existing_customer = session.exec(select(Customer).filter(func.lower(Customer.TaxNumber) == func.lower(model.TaxNumber))).first()
        if existing_customer:
...
def get_customer_by_id(session: Session, customer_id: str) -> CustomerResponseModel:
    customer = session.exec(select(Customer).filter(Customer.id == customer_id)).first()
    if not customer:
...
def update_customer(session: Session, customer_id: str, model: CustomerUpdateModel) -> CustomerResponseModel:
    ...
    session.exec(select(Customer).filter(Customer.id == customer_id).update(update_data))
...
def search_customers(session: Session, filter: CustomerSearchFilter) -> CustomerSearchResults:
    ...
    items = list(map(lambda x: x.dict(), customers))

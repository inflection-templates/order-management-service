...
from app.database.sql_model.models.order_type import OrderType
...
def update_order_type(session: Session, order_type_id: str, model: OrderTypeUpdateModel) -> OrderTypeResponseModel:
    ...
    session.execute(
        select(OrderType).where(OrderType.id == order_type_id).update(update_data)
    )
...
def search_order_types(session: Session, filter: OrderTypeSearchFilter) -> OrderTypeSearchResults:
    query = session.exec(select(OrderType))
    ...
    items = list(map(lambda x: x.dict(), orderTypes))

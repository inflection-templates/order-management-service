...
from app.database.sql_model.models.payment_transaction import PaymentTransaction
...
def get_payment_transaction_by_id(session: Session, payment_transaction_id: str) -> PaymentTransactionResponseModel:
    payment_transaction = session.exec(select(PaymentTransaction).where(PaymentTransaction.id == payment_transaction_id)).first()
...
def search_payment_transactions(session: Session, filter: PaymentTransactionSearchFilter) -> PaymentTransactionSearchResults:
    ...
    query = session.exec(select(PaymentTransaction))
    ...
    items = [payment_transaction.dict() for payment_transaction in paymentTransactions]

...
from app.database.sql_model.models.address import Address
...
def update_address(session: Session, address_id: str, model: AddressUpdateModel) -> AddressResponseModel:
    ...
    session.execute(
        select(Address).where(Address.id == address_id).update(update_data, synchronize_session="auto")
    )
...
def search_addresses(session: Session, filter: AddressSearchFilter) -> AddressSearchResults:
    query = session.exec(select(Address))
    ...
    items = list(map(lambda x: x.dict(), addresses))

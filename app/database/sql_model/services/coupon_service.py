...
from app.database.sql_model.models.coupon import Coupon
...
def get_coupon_by_id(session: Session, coupon_id: UUID) -> CouponResponseModel:
    coupon = session.exec(select(Coupon).where(Coupon.id == coupon_id)).first()
    if not coupon:
        raise NotFound(f"Coupon with id {coupon_id} not found")
    return coupon.dict()

@trace_span("service: update_coupon")
def update_coupon(session: Session, coupon_id: UUID, model: CouponUpdateModel) -> CouponResponseModel:
    coupon = session.exec(select(Coupon).where(Coupon.id == coupon_id)).first()
    if not coupon:
        raise NotFound(f"Coupon with id {coupon_id} not found")
    ...
    session.exec(
        select(Coupon).where(Coupon.id == coupon_id).update(update_data, synchronize_session="auto")
    )
    session.commit()
...
def search_coupons(session: Session, filter: CouponSearchFilter) -> CouponSearchResults:
    ...
    query = session.exec(select(Coupon))
    ...
    items = list(map(lambda x: x.dict(), coupons))
...
def delete_coupon(session: Session, coupon_id: UUID):
    coupon = session.exec(select(Coupon).where(Coupon.id == coupon_id)).first()
    if not coupon:
        raise NotFound(f"Coupon with id {coupon_id} not found")
